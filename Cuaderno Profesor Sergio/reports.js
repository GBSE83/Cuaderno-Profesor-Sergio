...
import { handleLoadBackup, handleSaveBackup, modalConfirm, modalAlert, modalOptions } from './utils/backup-utils.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const MAX_SAVED_CONFIGURATIONS = 15;
    ...
    const renderSavedConfigurationsList = () => {
        ...
        sortedConfigs.forEach(config => {
            ...
            loadButton.addEventListener('click', () => applyReportConfiguration(config));
            configActions.appendChild(loadButton);

            const deleteButton = document.createElement('button');
            deleteButton.textContent = 'Eliminar';
            deleteButton.classList.add('action-button-small', 'delete-config-button');
            deleteButton.addEventListener('click', () => deleteReportConfiguration(config.id));
    ...
    const applyReportConfiguration = (config) => {
        ...
        selectedGroupKeys = config.groupKeys || [];
        // basic filters
        ...
        // NEW: Apply activity sort order from config. Clear manual order if non-manual sort is selected.
        activitySortOrder = config.activitySortOrder || 'date_desc';
        if (activitySortSelect) {
            activitySortSelect.value = activitySortOrder;
        ...
        // categories will be selected when options are built
        selectedActivityCategoryFilterValues = config.activityCategoryFilterValues || [];
        // NEW: restore selections and manual order for single group
        if (selectedGroupKeys.length === 1) {
            ...
            setSessionItem('temp_restoredActivityIndices', JSON.stringify(config.selectedActivityIndices || []));
            setSessionItem('temp_restoredStudentNames', JSON.stringify(config.selectedStudentNames || []));
        }
        // NEW: when applying a saved configuration, replace persisted activity selections with the config's selection (or clear)
        if (selectedGroupKeys.length === 1) {
            // store configured activity indices into session persistence so UI reflects the loaded configuration
            const persistedIndices = Array.isArray(config.selectedActivityIndices) ? config.selectedActivityIndices : [];
            sessionStorage.setItem(SELECTED_ACTIVITIES_KEY, JSON.stringify(persistedIndices));
        } else {
            // multiple groups: clear persisted activities when applying multi-group config
    ...
    const saveCurrentReportConfiguration = async () => {
        ...
        if (savedReportConfigurations.length >= MAX_SAVED_CONFIGURATIONS) {
            await modalConfirm('¿Quieres eliminar la configuración más antigua para guardar esta nueva?');
    ...
    if (saveConfigurationButton) {
        saveConfigurationButton.addEventListener('click', () => { saveCurrentReportConfiguration(); });
    }
    ...
    async function generateGradesXlsx() {
        ...
        for (const gk of selectedGroupKeys) {
            ...
            // Si no hay actividades seleccionadas y se incluye asistencia, omitir la hoja de calificaciones
            if (includeAttendanceCheckbox.checked && activityIndices.length === 0 && selectedGroupKeys.length === 1) {
                // continuar para crear solo la hoja de asistencia más abajo
            } else {
        ...
        const filenameBase = `Informe_Calificaciones_${formatDate(new Date())}.xlsx`;
