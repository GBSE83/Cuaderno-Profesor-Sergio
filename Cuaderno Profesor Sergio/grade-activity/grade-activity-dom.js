...
export const renderStudentGrades = (students, selectedActivity, studentGradesList, saveGradesButton) => {
    ...
    students.forEach(student => {
        ...
        listItem.innerHTML = `
            ...
            <span class="student-grade-item-name">${studentName}</span>
            <div class="grade-input-container">
                <label for="grade-${studentName}">Calificación:</label>
                ${gradeInputHtml}
            </div>
            <div class="grade-actions-container">
                ...
                <button class="observation-button ${savedObservation ? 'has-observation' : ''}" data-student-name="${studentName}">Observaciones</button>
            </div>
        `;
...
export const attachGradeInputListeners = (studentGradesList, setUnsavedChangesFlag) => {
    ...
    // Handle NP checkbox interaction
    studentGradesList.addEventListener('change', (event) => {
        const npCheckbox = event.target.closest('.np-checkbox');
        if (npCheckbox) {
            const listItem = npCheckbox.closest('li');
            const gradeInput = listItem.querySelector('.grade-input');
            const rubricInputs = listItem.querySelectorAll('.rubric-input');
            const totalEl = listItem.querySelector('.rubric-total');
            const incrementDecrementButtons = listItem.querySelectorAll('.increment-grade-button, .decrement-grade-button');

            if (npCheckbox.checked) {
                // Disable editing immediately and set visible representation to 'NP' where appropriate
                if (gradeInput) {
                    gradeInput.disabled = true;
                    if (gradeInput.tagName === 'SELECT') {
                        // If select has NP option, use it; otherwise clear and show a placeholder
                        const hasNP = Array.from(gradeInput.options).some(o => o.value === 'NP');
                        if (hasNP) gradeInput.value = 'NP';
                        else gradeInput.value = '';
                    } else {
                        // For numeric/text inputs, clear value visually and keep disabled
                        gradeInput.value = '';
                    }
                }
                rubricInputs.forEach(input => { input.disabled = true; });
                incrementDecrementButtons.forEach(button => { button.disabled = true; });
                if (totalEl) totalEl.textContent = 'NP';
                // NEW: If this list item contains mixed-controls (icon + number), disable both children
                const mixedControls = listItem.querySelector('.mixed-controls');
                if (mixedControls) {
                    mixedControls.querySelectorAll('select, input').forEach(el => el.disabled = true);
                }
            } else {
                // Re-enable editing immediately. Do not attempt to restore a previous value from activity object here —
                // allow the user to enter a new value straight away.
                if (gradeInput) {
                    gradeInput.disabled = false;
                    // If it is a select and currently set to 'NP', reset to default '-' so user can choose
                    if (gradeInput.tagName === 'SELECT' && gradeInput.value === 'NP') {
                        gradeInput.value = '-';
                    }
                }
                rubricInputs.forEach(input => { input.disabled = false; });
                incrementDecrementButtons.forEach(button => { button.disabled = false; });
                // NEW: Re-enable mixed-controls children if present
                const mixedControls = listItem.querySelector('.mixed-controls');
                if (mixedControls) {
                    mixedControls.querySelectorAll('select, input').forEach(el => el.disabled = false);
                }
                // Recalculate rubric total if present
                if (totalEl) {
                    const grid = listItem.querySelector('.rubric-grid');
                    if (grid) {
                        const rows = Array.from(grid.querySelectorAll('.rubric-input'));
                        const weights = Array.from(grid.querySelectorAll('.rubric-weight')).map(w => parseInt(w.textContent.replace('%','')||'0',10));
                        const scores = rows.map(i => parseInt(i.value||'0',10));
                        const total = weights.reduce((sum, w, idx) => sum + (w/100)*(scores[idx]||0), 0);
                        totalEl.textContent = total.toFixed(2);
                    } else {
                        // If no rubric present, leave numeric input value as-is (user may enter)
                    }
                }
            }
        }
    ...
    // Rubric inputs: update flag on input and try to update visible total live
    studentGradesList.addEventListener('input', (event) => {
        const rubricInput = event.target.closest('.rubric-input');
        if (rubricInput) {
            setUnsavedChangesFlag();
            
            const rubricTitleWithIcon = rubricInput.closest('.rubric-row')?.querySelector('.rubric-title-with-icon');
            if (rubricTitleWithIcon) {
                // Ensure the title attribute is accessible on the rubric-title-with-icon div,
                // so the tooltip appears when hovering over the text or the icon.
            }

            const grid = rubricInput.closest('.rubric-grid');
            if (grid) {
                const rows = Array.from(grid.querySelectorAll('.rubric-input'));
                const weights = Array.from(grid.querySelectorAll('.rubric-weight')).map(w => parseInt(w.textContent.replace('%','')||'0',10));
                const scores = rows.map(i => parseInt(i.value||'0',10));
